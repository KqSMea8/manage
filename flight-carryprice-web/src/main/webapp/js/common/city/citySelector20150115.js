...
/* cityname数据源来源于cityname.js文件 */
Vcity.allCity = citysname;
/*
Vcity.allCity = ['北京|beijing|bj','上海|shanghai|sh', '重庆|chongqing|cq',  '深圳|shenzhen|sz', '广州|guangzhou|gz', '杭州|hangzhou|hz',
    ...
    '张家界|zhangjiajie|zjj', '舟山|zhoushan|zs|sdf', '郑州|zhengzhou|zz', '中卫|zhongwei|zw', '芷江|zhijiang|zj', '湛江|zhanjiang|zj'];
*/
/* 正则表达式 筛选中文城市名、拼音、首字母 */

Vcity.regEx = /^([\u4E00-\u9FA5\uf900-\ufa2d]+)\|(\w+)\|(\w)\w*\|(\w*)$/i;
Vcity.regExChiese = /([\u4E00-\u9FA5\uf900-\ufa2d]+)/;
Vcity.regExEnglish = /([^\u4E00-\u9FA5\uf900-\ufa2d]+)/;
Vcity.regExThreecode = /(\|[a-zA-Z]{3})/;
    ...
    if (!Vcity.oCity) {
        ...
        for (var i = 0, n = citys.length; i < n; i++) {
            ...
            if (reg2.test(letter)) {
                ...
                Vcity.oCity.ABCDEFGH[letter].push(match[1]+match[4]);
            } else if (reg3.test(letter)) {
                if (!Vcity.oCity.IJKLMNOP[letter]) Vcity.oCity.IJKLMNOP[letter] = [];
                Vcity.oCity.IJKLMNOP[letter].push(match[1]+match[4]);
            } else if (reg4.test(letter)) {
                if (!Vcity.oCity.QRSTUVWXYZ[letter]) Vcity.oCity.QRSTUVWXYZ[letter] = [];
                Vcity.oCity.QRSTUVWXYZ[letter].push(match[1]+match[4]);
            }
            /* 热门城市 前16条 */
            if(i<16){
...
/* 城市HTML模板 */
Vcity._template = [
    '<p class="tip">热门城市(支持汉字/拼音/三字码)</p>',
    '<ul>',
    '<li class="on">热门城市</li>',
    '<li>ABCDEFGH</li>',
    '<li>IJKLMNOP</li>',
...
Vcity.CitySelector.prototype = {
    ...
    initialize :function (options) {
        ...
        this.inputHidden = Vcity._m.$('#'+ inputHidden);
        this.inputEvent();
    },
        ...
        for(key in oCity){
            ...
            for(var j=0,k = sortKey.length;j<k;j++){
                ...
                for(var i=0,n=oCity[key][sortKey[j]].length;i<n;i++){
                	// 中文名称
                	var citynameC = Vcity.regExChiese.exec(oCity[key][sortKey[j]][i])[0];
                	// 英文三字码
                	var citynameE = Vcity.regExEnglish.exec(oCity[key][sortKey[j]][i])[0];
                    str = '<a href="javascript:" value="'+ citynameE +'">' + citynameC +'</a>';
                    odda.push(str);
                }
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                // var threecode = this.getAttribute("value");
                that.inputHidden.value = this.getAttribute("value");
               ...
               that.input.value = '';
	            ...
	            if(that.isEmpty){
		           ...
		           var inputValue = that.ul.childNodes[0].childNodes[0].innerHTML;
		            // ul中的第一个li的城市三字码
		           var hiddenValue = that.ul.childNodes[0].childNodes[2].innerHTML;
		           if(hiddenValue.indexOf("|")>-1){
		              hiddenValue = hiddenValue.replace("|","");
		           }
                   that.input.value = inputValue;
                   that.inputHidden.value = hiddenValue.trim();
	            }
            }
        });
            ...
            for (var i = 0, n = Vcity.allCity.length; i < n; i++) {
                if (reg.test(Vcity.allCity[i])) {
                    ...
                    if (searchResult.length !== 0) {
                        str = '<li><b class="cityname">' + match[1] + '</b><b class="cityspell">' + match[2] + '</b><b class="citythreecode" style="display:none">|'+ match[4] +'</b></li>';
                    } else {
                        str = '<li class="on"><b class="cityname">' + match[1] + '</b><b class="cityspell">' + match[2] + '</b><b class="citythreecode" style="display:none">|'+ match[4] +'</b></li>';
                    }
    ...
    KeyboardEvent:function(event,keycode){
        ...
        switch(keycode){
            ...
            case 13: // enter键
                ...
                this.inputHidden.value = Vcity.regExThreecode.exec(lis[this.count].innerHTML)[0].substring(1);
                Vcity._m.addClass('hide',this.ul);
                Vcity._m.addClass('hide',this.ul);
                /* IE6 */
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){
                ...
                that.inputHidden.value = Vcity.regExThreecode.exec(target.innerHTML)[0].substring(1);
